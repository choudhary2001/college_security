...
def send_mail():
    ...
    port = 587
    sender_email = 'adityabhargav2111@gmail.com'
    password = 'Singh@1234'
    recipient_email = 'divakarkumarhy2001@gmail.com'
    message = 'Someone is coming inside who is not from the college '
    try:
...
lbl3 = tk.Label(window, text="Branch",
                width=20, fg="green", bg="white",
                height=2, font=('times', 15, ' bold '))
lbl3.place(x=400, y=280)
 
txt3 = tk.Entry(window, width=20,
                bg="white", fg="green",
                font=('times', 15, ' bold '))
txt3.place(x=700, y=305)

lbl4 = tk.Label(window, text="Batch",
                width=20, fg="green", bg="white",
                height=2, font=('times', 15, ' bold '))
lbl4.place(x=400, y=320)
 
txt4 = tk.Entry(window, width=20,
                bg="white", fg="green",
                font=('times', 15, ' bold '))
txt4.place(x=700, y=345)
...
def TakeImages():
    ...
    if(is_number(Id) and name.isalpha() and branch.isalpha()):
        ...
        while(True):
            ...
            for (x, y, w, h) in faces:
                ...
                if(conf < 50):
                    ...
                    cv2.putText(img, "ALready Exist.", (x, y + h),
                        font, 1, (255, 255, 255), 2)
                    res = "Student is already exist."
                    print(res)
            ...
            if(conf < 50):
                if len(face_data)>0 :
                    if Id != face_data[-1]:
                        print(Id)
                        print(df[df['Id'] == Id])
                        aa = str(df.loc[df['Id'] == Id]['Name'].values).strip("['']")
                        ab = str(df.loc[df['Id'] == Id]['Branch'].values).strip("['']")
                        abc = str(df.loc[df['Id'] == Id]['Batch'].values).strip("['']")
                        tt = str(Id)+"-"+aa+"-"+ab
                        print(aa)
                        print(ab)
                        date_time = datetime.datetime.now()
                        res = "Attendance added for ID : " + str(Id) + " Name : " + aa
                        row = [Id, aa, ab, abc, "Present",date_time]
                        with open('UserDetails\\UserAttendance.csv', 'a+') as csvFile:
                            writer = csv.writer(csvFile)
                            # Entry of the row in csv file
                            writer.writerow(row)
                        csvFile.close()
